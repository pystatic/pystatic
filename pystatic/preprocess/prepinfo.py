...
class PrepInfo:
    ...
    def add_local_def(self, node: AssignNode, is_method: bool, mbox: "MessageBox"):
        def is_strong_def(node: AssignNode):
            ...
            definition.
            """
            return getattr(node, "type_comment", None) or getattr(
    ...
    def add_import_def(
        self, node: Union[ast.Import, ast.ImportFrom], infolist: List["prep_impt"]
    ):
    ...
    def add_typevar_def(self, name: str, typevar: "TypeVarIns", defnode: AssignNode):
        assert name in self.local
        self.typevar.add(name)

    def add_type_alias(self, alias: str, typealias: TypeAlias, defnode: AssignNode):
    ...
    def dump(self):
        ...
        for name, impt in self.impt.items():
            value = impt.getins()
            self.symtable.import_cache.add_cache(impt.symid, impt.origin_name, value)
...
class PrepEnvironment:
    ...
    def try_add_target_prepinfo(self, target: "BlockTarget", prepinfo: "PrepInfo"):
...
class prep_cls:
    def __init__(
        ...
        clstemp: "TypeClassTemp",
        prepinfo: "PrepInfo",
        def_prepinfo: "PrepInfo",
        defnode: ast.ClassDef,
    ) -> None:
