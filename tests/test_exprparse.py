...
def parse_eval_ann_check(expr: str, module_ins, expect: TypeIns):
    astnode = parse_expr(expr)
    eval_option = infer_expr(astnode, module_ins)
    eval_res = eval_option.value
...
def test_exprparse_type_expr():
    ...
    a = manager.infer_expr(src, "a")
    b = manager.infer_expr(src, "b")
    c = manager.infer_expr(src, "c")
    d = manager.infer_expr(src, "d")
    e = manager.infer_expr(src, "e")
    f = manager.infer_expr(src, "f")
    g = manager.infer_expr(src, "g")
    A = manager.infer_expr(src, "A")
...
def test_exprparse1():
    ...
    str_typetype = manager.infer_expr("builtins", "str")
    assert str_typetype
    ...
    cmp_res_option = infer_expr(cmp_node, module_ins)
